...
def compute_snr(noise_scheduler, timesteps):  # copy from higher version of diffuser
    """
    Computes SNR as per
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, validation_prompts, step):
...
def main():
    ...
    optim_param = list(unet.parameters())
    ...
    train_transform = transforms.Compose([
        transforms.Resize((args.resolution, args.resolution), Image.LANCZOS)
    ])
    train_dataset = BaseDataset(args.dataset_folder, json_file_name=args.json_file_name, transform=train_transform, tokenizer=tokenizer)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                init_image = batch["pixel_values"].to(weight_dtype)
