...
def parse_args():
    parser = argparse.ArgumentParser(description="Example of testing on UGRC using Gemini.")
    parser.add_argument(
        "--api_key", type=str, required=True, help="Your gemini api key",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_path", type=str, default="annotations_gemini.json", help="save path for predicted annotations",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    prompt = "Detect the 2d bounding boxes of all the cars."  # Gemini 2.0 prompt

    for image_ann in tqdm(image_list):
        img = Image.open(os.path.join(test_data_base_path, "images", image_ann["file_name"]))

        response = model.generate_content([img, prompt],request_options={"timeout": 10000})
